    ...
    <div class="text-black">
        ...
        <div>
            <div>
                ...
                <Table :columns="col" :td="tableData" :buttonTitle="'Verify'" :tableFunc="verifySupplier" />
            </div>
        </div>
        <div>
            <div>
                ...
                <Table :columns="colMedicine" :td="tableDataMedicine" :buttonTitle="'Verify'" :tableFunc="verifyMedicine"  />
            </div>
        </div>
        <div>
            <div>
                ...
                <Table :columns="colManufacturer" :td="tableDataManufacturer" :buttonTitle="'Verify'" :tableFunc="verifyManufacturer" />
            </div>
        </div>
        <UModal
        v-model="isOpen"
...
const col = ref([])
const tableData = ref([])
const colManufacturer = ref([])
const tableDataManufacturer = ref([])
...
const getAllManufacturerList = async()=>{
    ...
    manufacturerList.map((data) => {
        ...
        filteredKeys.forEach((data) => {
            ...
            if (!colManufacturer.value.some(item => item.key === colObj.key)) {
                colManufacturer.value.push(colObj);
            }
        ...
        tableDataManufacturer.value.push(rowData)

    })
    colManufacturer.value.push({key:'action',label:'Action'})
}

const verifyManufacturer = async(row)=>{
    // console.log('verify row data', row)
    const accounts = await window.ethereum.request({
      method:'eth_requestAccounts'
    })
    ...
    const txObject = {
      ...
      data: contractScm.methods.verifyManufacturer(
        row.walletAddress
...
const verifySupplier =async (supplierRow) =>{
    console.log("supplier dt", supplierRow)
    const accounts = await window.ethereum.request({
      method:'eth_requestAccounts'
    })
    const txObject = {
      from: accounts[0],
      to: contractScm.options.address,  // Smart contract address
      data: contractScm.methods.verifySupplier(
        supplierRow.walletAddress
      ).encodeABI(),  // ABI encoding of the method and parameters
    };
...
const verifyMedicine = async (medRow)=>{
    const accounts = await window.ethereum.request({
      method:'eth_requestAccounts'
    })
    // console.log('date is ',row)
    const txObject = {
      from: accounts[0],
      to: contractScm.options.address,  // Smart contract address
      data: contractScm.methods.verifyMedicine(
        medRow.medId
      ).encodeABI(),  // ABI encoding of the method and parameters
    };
