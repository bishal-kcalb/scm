<template>
    <div class=" flex items-center justify-center p-4">
      <div  class="max-w-4xl w-full mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <label for="manufacturerAddress" class="block text-sm font-medium text-gray-700">Medicine Id </label>
            <input id="manufacturerAddress" disabled  v-model="props.medId" required rows="3"
          ...
          <div class="space-y-2 md:col-span-2">
            ...
            <input id="manufacturerAddress" v-model="supplierAddress" required rows="3"
              class="mt-1 block text-black w-full bg-transparent rounded-sm border-black  focus:border-[#FF6600] focus:ring focus:ring-[#FF6600] focus:ring-opacity-50">
          </div>
        </div>
        <ButtonPrimary :title="title" class="mt-5" @click="sellMedicine()"/>
      </div>
    </div>
  </template>
  ...
  const sellMedicine = async()=>{
      ...
      console.log(supplierAddress.value)
      const txObject = {
        from: accounts[0],
        to: contractScm.options.address,  // Smart contract address
        data: contractScm.methods.sellMedToSupplier(
    ...
    //   console.log("hello",accounts[0],formData.value.supplierAddress.toLowerCase() )
